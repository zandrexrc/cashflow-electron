...
import {Account, NewAccount, Transaction} from '../types';
...
function getAccountName(accountId: number) {
  const accounts: Account[] = store.getState().accounts;
...
function getAccountNames() {
  ...
  for (let i = 0; i < accounts.length; i++) {
    let index = accounts[i].accountId;
    if (index !== null) {
 ...
 */
function calcMostUsedAccounts(accounts: Account[], transactions: Transaction[]) {
  ...
  const sortedAccounts: string[] = Object.keys(accountsCounter).sort(function(a, b) {
